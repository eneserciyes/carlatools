...
class CarlaServerManager():
    def __init__(self, carla_sh_str, port=2000, tm_port=8000, configs=None, t_sleep=5):
        ...
        if configs is None:
            ...
            for i, cfg in enumerate(configs):
                single_env_cfg = dict()##TODO: add OmegaConf functionality ## OmegaConf.to_container(cfg)
                single_env_cfg['gpu'] = cfg['gpu']
                single_env_cfg['port'] = port + i*5
                single_env_cfg['tmport'] = tm_port + i*5
    ...
    def start(self):
        ...
        for cfg in self.env_configs:
            cmd = f'CUDA_VISIBLE_DEVICES={cfg["gpu"]} bash {self._carla_sh_str} ' \
                f'-fps=10 -quality-level=Epic -carla-rpc-port={cfg["port"]} --trafficManagerPort={cfg["tmport"]}'
